...
function EMMark({ x, y, fillP, breathe }) {
  ...
  return (
    <div style={{
      ...
      filter: `drop-shadow(0 0 ${7 + breathe * 5}px rgba(40,200,180,${0.3 + breathe * 0.18}))`,
    }}>
...
function Wordmark({ x, yCenter, onWidth }) {
  ...
  React.useEffect(() => {
    ...
    const measure = () => onWidth(ref.current.getBoundingClientRect().width);
...
function LaunchpadReveal() {
  ...
  return (
    <div data-screen-label={`t=${t.toFixed(1)}s`} style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <EMMark x={emX} y={emY} fillP={fillP} breathe={breathe} />
